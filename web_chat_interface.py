...
💡 Tip: Try "how many tasks has Agent A completed in the last 3 days?" to get started.`,
        ...
        if (isCloudDeployment) {
            ...
            console.log('🏠 Local development detected - attempting WebSocket with aggressive fallback');
            // Connect on page load for local development
            connect();
            
            // Aggressive fallback - enable interface quickly regardless of WebSocket status
            setTimeout(() => {
                if (!socket || socket.readyState !== WebSocket.OPEN) {
                    console.log('WebSocket not ready after 1 second, enabling fallback mode for local dev');
                    enableFallbackMode();
                }
            }, 1000);
            
            // Backup fallback in case the first one fails
            setTimeout(() => {
                const messageInput = document.getElementById('messageInput');
                if (messageInput && messageInput.disabled) {
                    console.log('🚨 Emergency fallback - force enabling interface');
                    enableFallbackMode();
                }
            }, 3000);
        }
